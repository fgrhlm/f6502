...
void init_emu_test(emu* e, int argc, char* argv[]){
    ...
    debug_out(LOG_FILE,"[TEST %s - %s.json]\n", argv[argc-1], argv[argc-2]);
...
void emu_run(emu* e){
    ...
    while(emu_get_mode(e) != STOP){
        ...
        if(get_pc(c) == e->test_stop){ emu_set_mode(e, STOP); }
        if(e->step > 300){ debug_out(LOG_FILE, "Took too long..\n"); emu_set_mode(e, STOP); }
    }

    debug_out(LOG_FILE, "Done..\n");
}

int main(int argc, char* argv[]){
    debug_out_init(LOG_FILE); 
