...
void print_cycles(test_def t) {
    for (int i = 0; i < t.cycles_len; i++) {
        printf("(%d, %d, %d) ", t.cycles[i].addr, t.cycles[i].val,
               t.cycles[i].t);
    }
    printf("\n");
}
...
void bin_str(uint8_t b, char *str) {
    sprintf(str, "%d%d%d%d%d%d%d%d", (b & 0x80) ? 1 : 0, (b & 0x40) ? 1 : 0,
            (b & 0x20) ? 1 : 0, (b & 0x10) ? 1 : 0, (b & 0x08) ? 1 : 0,
            (b & 0x04) ? 1 : 0, (b & 0x02) ? 1 : 0, (b & 0x01) ? 1 : 0);
}

void test_assert(int index, char *n, uint8_t x, uint8_t y, test_result *t,
                 uint8_t verbose) {
    if (x != y) {
        *t = TEST_FAIL;
        if (verbose) {
...
void dump_cpu(cpu *c) {
    printf("    PH:%d PL:%d S:%d A:%d X:%d Y:%d P:%d\n", *get_reg(c, REG_PH),
           *get_reg(c, REG_PL), *get_reg(c, REG_S), *get_reg(c, REG_A),
           *get_reg(c, REG_X), *get_reg(c, REG_Y), *get_reg(c, REG_P));
}

test_result run_test(test_def *tests, int index, cpu *c, mem *m,
                     uint8_t verbose) {
    ...
    test_assert(index, "REG_PH", (t.final.pc >> 8), *get_reg(c, REG_PH), &res,
                verbose);
    test_assert(index, "REG_PL", (t.final.pc & 0x00FF), *get_reg(c, REG_PL),
                &res, verbose);
