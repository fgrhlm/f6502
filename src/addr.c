...
uint16_t absolute_addr(cpu *c, mem *m) {
    ...
    lo = mem_read(m, inc_pc(c));
    hi = mem_read(m, inc_pc(c));
...
uint16_t absolute_indirect_addr(cpu *c, mem *m) {
    ...
    lo = mem_read(m, abs_addr);
    hi = mem_read(m, abs_addr + 1);
...
uint16_t zero_page_addr(cpu *c, mem *m) {
    ...
    switch (get_addr_mode(c)) {
    case AM_ZERO_PAGE:
        byte = mem_read(m, inc_pc(c));
        addr = (0 << 8) | byte;
        break;
    case AM_ZERO_PAGE_X:
        byte = mem_read(m, inc_pc(c));
...
uint16_t zero_page_indirect_x(cpu *c, mem *m) {
    ...
    uint8_t byte = mem_read(m, inc_pc(c));

    t_addr = byte + reg;
    lo = mem_read(m, (uint16_t)t_addr);

    t_addr = (byte + 1) + reg;
    hi = mem_read(m, (uint16_t)t_addr);
...
uint16_t zero_page_indirect_y(cpu *c, mem *m) {
    uint8_t byte_addr = mem_read(m, inc_pc(c));
    uint8_t byte = mem_read(m, (0 << 8) | byte_addr);
    ...
    byte_addr += 1;
    hi = mem_read(m, (0 << 8) | byte_addr) + carry;
